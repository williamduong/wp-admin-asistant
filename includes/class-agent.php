...
13. If you include Mermaid, output only raw Mermaid syntax inside the shortcode — never wrap it in triple backticks, never mix explanation text into the shortcode, and prefer short conservative diagrams that are likely to parse on Mermaid v11. Use plain `flowchart TD` by default unless another type is clearly needed. Prefer simple square-bracket nodes like `A[Label]` and `B{Decision}`; avoid round `A(Label)` nodes.
